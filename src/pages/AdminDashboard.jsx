...
export default function AdminDashboard({ view: routeView }) {
  ...
  return (
    <div className="h-screen w-screen bg-[#070e1c] p-1 sm:p-2.5 md:p-3.5 font-['Inter',sans-serif] text-slate-800 antialiased flex flex-col justify-center overflow-hidden">
      {/* Toast Notification */}
      {toast && (
        <div
          className={`fixed top-4 sm:top-5 right-4 sm:right-5 z-50 flex items-center gap-3 rounded-2xl border ${
            toast.type === 'error'
          ...
          } p-3.5 sm:p-4 max-w-md w-[calc(100vw-2rem)] sm:w-auto transition-all backdrop-blur-md animate-fadeIn`}
        >
      ...
      <div className="w-full h-full rounded-2xl sm:rounded-[32px] md:rounded-[40px] shadow-2xl shadow-slate-950/70 overflow-hidden bg-white flex flex-col lg:flex-row border border-slate-800/40 relative">
        {/* Left Sleek Executive Dark Slate Sidebar (Drawer on mobile, fixed on desktop) */}
        <AdminSidebar
          view={view}
        ...
        <div className="flex-1 h-full flex flex-col min-w-0 overflow-hidden bg-white">
          ...
          <main className="flex-1 p-3 sm:p-5 lg:p-8 space-y-4 sm:space-y-6 overflow-y-auto pb-16 bg-white">
          ...
          <footer className="px-3 sm:px-6 py-2 sm:py-2.5 bg-white/95 backdrop-blur-md border-t border-slate-200/80 flex flex-wrap items-center justify-between gap-2 text-[11px] sm:text-xs text-slate-500 shrink-0 select-none z-10 shadow-xs">
            <div className="flex items-center gap-2">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <span className="font-medium text-slate-700">Baba Broker Desk</span>
              <span className="text-[10px] text-slate-400 font-mono hidden sm:inline">v2.4</span>
            </div>
            <div className="flex items-center gap-2 sm:gap-4 text-slate-400">
              ...
              <span className="text-slate-400 text-[10px] sm:text-xs">© {new Date().getFullYear()} Baba Broker</span>
            </div>
          </footer>
        </div>
      </div>
